...
impl StateValue {
    ...
    fn compute_rapid_hash(data: &Bytes) -> Option<u64> {
        if data.len() >= RAPID_HASH_THRESHOLD {
            Some(rapidhash(data))
        } else {
            None
        }
    ...
    fn to_persistable_form(&self) -> PersistedStateValue {
        let Self { data, metadata, maybe_rapid_hash: _ } = self.clone();
    ...
    pub fn unpack(self) -> (StateValueMetadata, Bytes) {
        let Self { data, metadata, maybe_rapid_hash: _ } = self;
