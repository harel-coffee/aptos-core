...
module sigma_protocols::homomorphism {
    ...
    use sigma_protocols::utils::{equal_vec_points, add_vec_points, compress_points, points_clone,
        mul_points, neg_scalars
    };
    ...
    #[test_only]
    use sigma_protocols::utils::{add_vec_scalars, mul_scalars};
        ...
        m: u64): (Scalar, vector<Scalar>)
    {
        ...
        // TODO(Security): May be good to hash in $k$? A hardcodes $m$ and the statement hardcodes $n_1$ and $n_2$.
    ...
    public inline fun prove(
        ...
        let m = _A.length();
        let (e, _) = fiat_shamir(dst, stmt, &_A, m);
    ...
    public inline fun verify(
        ...
        let _A = proof.get_commitment();
        let m = _A.length();
        let (e, betas) = fiat_shamir(dst, stmt, _A, m);
